...
def create_model(state_shape, kernel_dims, num_actions, filters_per_layer):
	...
	layer_5 = layers.Conv2D(filters = filters_per_layer, kernel_size = kernel_dims, padding = "same", activation="relu", kernel_initializer = initializer)(norm_4)
	norm_5	= layers.BatchNormalization()(layer_5, training = True)
	
	#relu because outputs are relative probabilities and predicted future rewards
	actions = layers.Conv2D(filters = num_actions, kernel_size = (3,3), padding = "same", activation = "relu")(norm_5)
