...
from _map_image import indx_map_x_white
...
RANDOM_WEIGHTS			= ["Left"]*2 + ["None"] + ["Right"]
...
# -- CREATE MODELS --	
model					= create_model(STATE_SHAPE, KERNEL_DIMS, NUM_ACTIONS, FILTERS_PER_LAYER)			# action probabilities -> environment -> calculated rewards
target_model			= create_model(STATE_SHAPE, KERNEL_DIMS, NUM_ACTIONS, FILTERS_PER_LAYER)			# reward predictions.  used with model to calculate losses
...
while epoch_iter < NUM_EPOCHS:
	...
	for image_ndx, left_ref in enumerate(left_imgs):
		...
		for step in range(1, MAX_STEPS_PER_IMAGE):
			...
			next_recreation		= indx_map_x_white(right_src, map)
			next_state			= env_state(left_ref, right_src, map, next_recreation)
			
			# -- CALC REWARD --
			# whited
			equal2condition = tf.math.equal(next_recreation, 0.0)
			next_recreation_oned = tf.where(equal2condition, 1.0, next_recreation)
			reward, reward_reduced = similarity_perpixel(left_ref, next_recreation_oned)
			...
			if step % UPDATE_AFTER_ACTIONS == 0 and len(action_history) > 2 * BATCH_SIZE:
				#should do a random selection not just the last batch_size
				state_next_sampe				= state_next_history[-BATCH_SIZE:]  
				reward_sample					= reward_history[-2 * BATCH_SIZE: - BATCH_SIZE] 
				action_sample 					= action_history[-2 * BATCH_SIZE: - BATCH_SIZE] 
				state_sample 					= state_history[-2 * BATCH_SIZE: - BATCH_SIZE] 
...
model_name = "white_32fut_5epoch_4Inputs_5LayerNormed_" + str(KERNEL_DIM) + "KERNEL_" + str(FILTERS_PER_LAYER) + "FILTERS_" + str(LEARNING_RATE) + "LR"
