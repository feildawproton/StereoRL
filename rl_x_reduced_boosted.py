...
from _similarity_error import similarity_perpixel_boosted
...
while epoch_iter < NUM_EPOCHS:
	...
	for image_ndx, left_ref in enumerate(left_imgs):
		...
		reward, reward_reduced	= similarity_perpixel_boosted(left_ref, right_src, BOOST)
		...
		for step in range(1, MAX_STEPS_PER_IMAGE):
			...
			# -- CALC REWARD --
			reward, reward_reduced = similarity_perpixel_boosted(left_ref, next_recreation, BOOST)
...
model_name = "0p9Boost_5epoch_4Inputs_5LayerNormed_" + str(KERNEL_DIM) + "KERNEL_" + str(FILTERS_PER_LAYER) + "FILTERS_" + str(LEARNING_RATE) + "LR"
